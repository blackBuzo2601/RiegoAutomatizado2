...
if not os.path.exists('datarealtime.json'):
    with open('datarealtime.json', 'w') as json_file:
...
while True:
    ...
    try:
        ...
        with open('datarealtime.json', 'r+') as json_file:
            #Si el archivo esta vacio, inicializar como una lista vacia.
            if os.stat('datarealtime.json').st_size == 0:
                data = []  
            else:
